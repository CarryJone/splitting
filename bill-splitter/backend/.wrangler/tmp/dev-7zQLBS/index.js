...
// .wrangler/tmp/bundle-aFnT80/middleware-loader.entry.ts
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();

// .wrangler/tmp/bundle-aFnT80/middleware-insertion-facade.js
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
...
var getPool = /* @__PURE__ */ __name(() => {
  if (!pool) {
    ...
    pool = new Pool({
      ...
      max: 1,
      // Serverless best practice: limit connections
...
var query = /* @__PURE__ */ __name(async (text, params) => {
...
// src/services/debtSimplification.ts
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
function calculateSettlement(members, expenses, splits) {
  const balances = {};
  ...
  Object.entries(balances).forEach(([idStr, amount]) => {
    ...
    if (amount < -1) debtors.push({ memberId: id, amount });
...
__name(calculateSettlement, "calculateSettlement");
...
groups.post("/", async (c) => {
  ...
  try {
    ...
    const group3 = result.rows[0];
    return c.json(group3, 201);
  } catch (err) {
    console.error(err);
...
groups.get("/:id", async (c) => {
  ...
  try {
    ...
    const expensesWithSplits = expensesResult.rows.map((expense) => ({
      ...expense,
      splits: splitsResult.rows.filter((split) => split.expense_id === expense.id).map((split) => ({
        ...split,
        member_id: split.member_member_id
        // Map DB column to frontend expected prop
      }))
    }));
    return c.json({
      ...groupResult.rows[0],
...
groups.get("/:id/settlement", async (c) => {
  ...
  try {
    ...
    const splits = splitsRes.rows.map((row) => ({
...
app.route("/api/groups", groups_default);
var src_default = app;
